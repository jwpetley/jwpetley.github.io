---
layout: page
title: Welcome
subtitle: I am looking for a job!
sitemap:
  priority: 0.9
---

<div class="row">
  <div class="column">
    <img src="{{ '/assets/img/CIV_detection_fraction_square.png' | prepend: site.baseurl }}" alt="CIV Detection Fraction" style="width:100%">
  </div>
  <div class="column">
    <img src="{{ '/assets/img/me.jpg' | prepend: site.baseurl }}" alt="Me" style="width:100%">
  </div>
  <div class="column">
    <img src="{{ '/assets/img/jet_image.png' | prepend: site.baseurl }}" alt="Radio Jet" style="width:130%">
  </div>
</div>



<div id="describe-text">
	<p>I am a PhD researcher studying quasars, highly luminous black hole systems, at Durham University. My supervisor is Dr. Leah Morabito. You can view my research publications here - <a href="https://ui.adsabs.harvard.edu/search/p_=0&q=author%3A%22Petley%2C%20J.%20W.%22&sort=first_author%20desc%2C%20bibcode%20desc" target="_blank">ADS Search</a>. </p>

  <p> Contact me at jwpetley(at)gmail.com or james.w.petley(at)durham.ac.uk </p>

  <p>I am currently looking for a job to start in ~October 2024. I am interested in post-doctoral positions but also research software engineer roles too. I have advanced skills in radio interferometry and pipeline development.</p>

<p>I have several blogs on this website based on my PhD research and small programming projects. Please get in contact for any project ideas, job enquiries or just to have a chat about black holes or programming.</p>

<h1>Latest Blog Post</h1>
{% for post in site.posts limit:1 %}
<p>
<li class="post-teaser">
<a href="{{ post.url | prepend: site.baseurl }}">
{{ post.title }} --- {{ post.date | date: "%d %B %Y" }}
</a>
</li>
</p>

{% endfor %}

</div>
